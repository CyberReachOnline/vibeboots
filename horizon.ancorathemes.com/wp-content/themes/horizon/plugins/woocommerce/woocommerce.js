/* global jQuery, HORIZON_STORAGE */

jQuery( document ).on( 'action.ready_horizon', function() {
	...
	$document.on( 'action.resize_horizon', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.horizon_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		horizon_set_cookie( 'horizon_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	horizon_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function horizon_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						horizon_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function horizon_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function(response) {
						...
						horizon_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						horizon_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							HORIZON_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_horizon', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < horizon_window_scroll_top() + horizon_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		horizon_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					horizon_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					horizon_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		horizon_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			horizon_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function horizon_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = horizon_window_width() + ( horizon_window_height() < $document.height() || horizon_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = table_cart;

				if ( w < HORIZON_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= HORIZON_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
